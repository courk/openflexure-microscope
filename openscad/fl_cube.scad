...
module fl_cube_outer()
{
    ...
    chamfer_bottom_edge() union()
    {
        ...
        for (p = [
                 [ -w / 2 + roc * 3, roc, roc + 0.5 ], [ w / 2 - roc * 3, roc, roc + 0.5 ], [ 0, roc, w - roc ],
...
module fl_cube()
{
    ...
    difference()
    {
        union()
        {
            ...
            sequential_hull()
            {
                translate([ -inner_w / 2, bottom, 0 ]) cube([
                    inner_w, d, beamsplit[2] + beamsplit[1] - bottom - dichroic_t * sqrt(2)
                ...
                translate([ -inner_w / 2, by - 4, 4 + 2 * dichroic_t ]) cube([ inner_w, 2, d ]); // end of retaining
                                                                                                 // clip
            ...
            reflect([ 1, 0, 0 ]) translate([ -w / 2, bottom + 4, w ]) sequential_hull()
            {
                ...
                translate([ 0.5 + 2 * roc + 1.5 - 0.2 * (1 + sqrt(2)), 0, roc + 1.5 - 0.2 ]) rotate([ -90, 0, 0 ])
                    cylinder(r = 0.2, h = depth); // cube([2*roc + 1.5, depth, d]);
            }
        }
        // hole for the beam
        translate(beamsplit) rotate([ 90, 0, 0 ]) cylinder(r = 5, h = 999, center = true, $fn = 32);
        if (has_emission_filter)
        {
            // hole for the emission filter
            translate([ -emission_filter[0] / 2, bottom - roc * 1.5, beamsplit[2] - emission_filter[1] / 2 ])
                cube([ emission_filter[0], emission_filter[2], 999 ]);
        }
        // access hole for the dichroic
        translate(beamsplit) rotate([ -45, 0, 0 ]) translate([ 0, -dichroic[1] / 2, 0 ]) scale([ 1.1, 1, 1.9 ])
